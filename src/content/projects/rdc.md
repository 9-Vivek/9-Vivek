...
description: A human-facing workspace for authoring structured relational data and exploring it through deterministic queries and interchangeable read-only views.
...
1. A structured query selects entities, fields, and relationships.
2. The engine produces a presentation-independent result set.
3. A rendering specification describes how to display that result.
4. A renderer creates a read-only view.
...
The AI Systems Institute curriculum is the first domain planned for RDC. Its capabilities, knowledge concepts, modules, lessons, and interventions form a relational structure that is difficult to reason about in a conventional database interface. RDC can provide one authoring environment while deriving a capability dependency graph, a knowledge hierarchy, and impact queries from the same source data.
